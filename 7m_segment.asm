...
again:
	...
	CJNE A, #01, skip		; if the number of revolutions is not 10 skip next instruction
...
setDirection:
	...
	PUSH 04H			; save value of location 20H (first bit-addressable 
					;	location in RAM) on stack
	CLR A				; clear A
	MOV 04H, #0			; clear location 20H
	...
	CJNE A, 04H, changeDir		; | compare SW0 (LSB of A) with F0 (LSB of 20H)
...
finish:
	POP 04H				; get original value for location 20H from the stack
